...
int Card::getTag() { return cardTag; }
...
void Deck::clearDeck() { cardsLeft = 0; }
...
int Deck::deckSize() { return cardsLeft; }

bool operator==(const Card &c1,
                const Card &c2) { // this should work for NULL checks
  if (c1.cardTag ==
      c2.cardTag) { // just wanted to see if that would change anything since
                    // operator is declared as Card::operator==
    return true;
  }
  return false;
}
