...
{ //this should work for NULL checks
    if (c1.cardTag == c2.cardTag)
    { // just wanted to see if that would change anything since operator is declared as Card::operator==
        return true;
    }
    return false;
}
...
void Deck::push_front(const Card &c)
{ // push card to front of deck for tie
    auto it = cDeck.begin();
    cDeck.insert(it, c);
}
