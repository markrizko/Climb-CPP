...
public:
  ...
  void Turn();
  int compare(); // compares cards - returns 0 if invalid move - 1 if win - 2 if
                 // draw
  void select();
  void endGame();
  void checkWin();
  ...
  bool finalEncounter(); // face another king at the end of the game with your
                         // remaining cards
...
private:
  ...
  int ca, blackCount, redCount, na,
      score; // card advantage, number advantage and score
  bool ValidMove;
  bool initFlag;
  bool fEnc; // boolean for select and display statement that detects when final
             // encounter is being played
};
