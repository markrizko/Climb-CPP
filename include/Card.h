...
public:
  ...
  // bool operator ==(const Card& other); // it gave me an error when I tried to
  // pass one param :( bool operator==(const Card& c1, const Card& c2);
  friend bool operator!=(const Card &c1, const Card &c2);
  Card operator=(const Card &source);
  Card operator[](int index) { return this[index]; }
